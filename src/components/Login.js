...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Maps thrue user data from api and checks for a match
      ...
      if (!error?.response) {
        setErrorMessage(
          "No server response... try pressing login button again! (will be fixed)"
        );
  ...
  return (
    <div className=" flex flex-col w-full max-w-md max-h-sm justify-start p-5 rounded-xl drop-shadow-2xl bg-white ">
      <p
        ...
        className={
          errorMessage
            ? "bg-red-400 font-bold rounded-md  p-3 mb-4 text-red-800"
            : "hidden"
        }
      >
        {errorMessage}
      ...
      >
        ...
        <button className="mt-2 py-1.5 px-3 font-mono text-xs text-slate-200  w-fit rounded-full bg-pink-600">
          Logga in
        </button>
      </form>
    </div>
